...
rmse_results <- bind_rows(rmse_results,
                          data.frame(model = paste0("Regularized Movie, User, Genres, ",
                                                    "Rating Period and Date Effect Model"),
...
preds <- lapply(X = names(folds), FUN = function(f) {
  ...
  # Generates a harmless message "Joining, by = c(..."
  
  removed <- anti_join(temp, test_tmp)
  ...
  rmses <- sapply(X = lambdas, FUN = function(l) {
    ...
    if (any(is.na(predicted_ratings))) {
      ...
      warning(paste0("ERROR! Ensure that the userId and",
                     " movieId in the test_tmp are also in train_tmp!"))
  ...
  return(data.frame(lambda = lambdas[which.min(rmses)], rmse = min(rmses),
                    stringsAsFactors = FALSE))
